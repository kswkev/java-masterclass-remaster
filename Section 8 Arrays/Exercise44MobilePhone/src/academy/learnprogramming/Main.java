...
//NOTE:  If you get an error from the Evaluate class, it's most likely the constructor. Check if you've added a constructor or if the constructor has the right arguments.
    ...
    public static void main(String[] args) {
        ...
        Contact kev = new Contact("Kevin", "123");
        Contact pauline = new Contact("Pauline", "789");
        addContact(mobilePhone, kev);
        addContact(mobilePhone, pauline);
        addContact(mobilePhone, kev);

        Contact kevUpdate = new Contact("Kevin", "123456");
        mobilePhone.updateContact(kev, kevUpdate);

        mobilePhone.removeContact(kevUpdate);

    }

    private static void addContact(MobilePhone mobilePhone, Contact contact) {
        if (mobilePhone.addNewContact(contact)) {
            System.out.println("Contact " + contact.getName() + " added");
        } else {
            System.out.println("Contact " + contact.getName() + " failed to add");
        }
    }
}
